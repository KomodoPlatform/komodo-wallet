...
class TransactionHistoryBloc
  ...
  TransactionHistoryBloc({required KomodoDefiSdk sdk})
    ...
    on<TransactionHistoryAuthStateChanged>(_onAuthStateChanged);
    ...
    _authSubscription = _sdk.auth.watchCurrentUser().listen((user) {
      if (!isClosed) {
        add(const TransactionHistoryAuthStateChanged());
      }
    });
  }
  ...
  /// Clears all transaction subscriptions when auth state changes
  /// (logout or wallet switch). This prevents stale subscriptions from
  /// continuing to receive updates for the previous wallet's transactions.
  Future<void> _onAuthStateChanged(
    TransactionHistoryAuthStateChanged event,
    Emitter<TransactionHistoryState> emit,
  ) async {
    await _historySubscription?.cancel();
    _historySubscription = null;
    await _newTransactionsSubscription?.cancel();
    _newTransactionsSubscription = null;
    _processedTxIds.clear();

    // Reset to initial state when auth changes
    emit(const TransactionHistoryState.initial());
  }
}
