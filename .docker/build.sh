...
if [ -n "$FEEDBACK_API_KEY" ] && [ -n "$FEEDBACK_PRODUCTION_URL" ]; then
  ...
  if [ -n "$FEEDBACK_API_KEY" ] || [ -n "$FEEDBACK_PRODUCTION_URL" ]; then
    echo "Warning: Incomplete Cloudflare feedback credentials provided. Both FEEDBACK_API_KEY and FEEDBACK_PRODUCTION_URL must be present to include them in the build."
  fi
fi
